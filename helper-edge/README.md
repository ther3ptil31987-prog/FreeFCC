# FreeFCC Edge (`helper-edge/`)

A small, self-contained Android app (`com.freefcc.edge`) that adds a slim edge
handle to the DJI RC 2 controller. Tapping or swiping the handle opens a short,
user-chosen list of apps over DJI Fly and launches the selected one. It replaces
two problematic bundled helpers (`03_ATVLauncher`, `04_Edge Gestures`) with a
single MIT-clean build.

It is a separate Gradle project inside the FreeFCC repository; the main FreeFCC
app is not touched. See `FORK_NOTES.md` for how it derives from its pinned
reference (Smart-Edge) and `THIRD_PARTY_NOTICES` for licenses.

## Requirements

- JDK 17
- Android SDK (set `sdk.dir` in `helper-edge/local.properties`, or `ANDROID_HOME`)
...
Run everything from `helper-edge/`:
