# EdgeHatch — fork / derivation notes

`edgehatch/` is EdgeHatch, a small, self-contained Android edge launcher
(`app.edgehatch.launcher`): a slim screen-edge handle that opens a short list of
apps over the foreground app and launches one. It is a standalone, MIT-clean
build. DJI (the RC 2 controller, the DJI Fly app) is named only as an
unaffiliated compatibility reference — there is no partnership with DJI or any
other project.
...
Uses a pinned, checksum-verified toolchain: Gradle 8.10.2 (wrapper
